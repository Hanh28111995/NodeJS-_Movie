...
async createOneShowtime({ theaterId, movieId, startTime, cinemaId }) {
    ...
    const theaterDoc = await theaterRepository.findById ? await theaterRepository.findById(theaterId) : await Theater.findById(theaterId).lean();
    if (!theaterDoc) {
    ...
    const seatTypeIds = [...new Set((theaterDoc.seats || []).map((s) => s.seatType?.toString()).filter(Boolean))];
    const seatTypes = await seatTypeRepository.findByIds 
        ? await seatTypeRepository.findByIds(seatTypeIds) 
        : await SeatType.find({ _id: { $in: seatTypeIds } }).select("_id price color").lean();
