...
export const createOneShowtime = async ({ theaterId, movieId, startTime }) => {
  ...
  if (!theater.seats?.length)
    throw new Error("Phòng chiếu chưa có cấu hình ghế mặc định");

  const isExisted = await Showtime.findOne({ theater: theaterId, startTime });
  if (isExisted)
    throw new Error("Khung giờ này tại phòng chiếu đã có suất chiếu khác");

  const seatTypeIds = [
    ...new Set(
      theater.seats.map((s) => s.seatType?.toString()).filter(Boolean),
    ),
  ];
